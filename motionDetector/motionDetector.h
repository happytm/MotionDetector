...
int motionDetector_deinit();  // deinitializes the storage arrays


int motionDetector_config(int, int, int, int, bool); // reconfigure the library with new parameters: sample buffer depth (how many samples to store in the circular buffer, mobile average filter size, variance threshold ( >= 0, how much the interference signal deviates from the norm before triggering a detection result, in dBm), variance integrator limit (how many variance samples we cumulate before evaluating the variance threshold level), finally bolean var set to true -> enable autoregressive filtering (default is false -> disable autoregressive filtering)


int motionDetector_process(int); // receives RSSI signal as parameter, returns the detection level ( < 0 -> error (see ERROR LEVELS section), == 0 -> no detection, > 0 -> detection level in dBm)


int motionDetector(); // generic version, only works in STA mode; request the RSSI level internally, then process the signal and return the detection level in dBm
...
int motionDetector_esp(); // ESP32 specific version, uses the ESP API directly and also works in SoftAP mode; request the RSSI level internally, then process the signal and return the detection level in dBm
...
int motionDetector_debug_via_serial(int);  // parameter is debug level, set it to at least >= 1; the highest the level, the more messages you enjoy


int motionDetector_set_debug_level(int);  // parameter is debug level, set it to at least >= 1; the highest the level, the more messages you enjoy



// current status: IMPLEMENTED
int motionDetector_enable_serial_CSV_graph_data(int); // [ 0 = disabled, >=1 = enabled (default is 0) ] completely disable the verbose output and only print the variance over the serial console, so that any program listening to the serial can graph the relevant data. 


// current status: IMPLEMENTED
int motionDetector_set_minimum_RSSI(int);

// current status: IMPLEMENTED
int motionDetector_enable_alarm(int);


// current status: IMPLEMENTED
int motionDetector_set_alarm_threshold(int);
